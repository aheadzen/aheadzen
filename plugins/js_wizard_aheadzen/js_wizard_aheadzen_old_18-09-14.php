...
if(!function_exists('wpw_template_include'))
{
	...
	function wpw_template_include($template)
	{
		...
		if(isset($_GET['themeactivated']))
		{
			...
			//if($_SESSION['selected_theme']==$selected_theme)
	...
	function aheadzen_register_form_shortcode( $atts, $content = null ) {
		...
		if (!is_user_logged_in()) {
			...
			if($_SESSION['emsg_array'])
			{
				...
				echo '<div class="errormsg">'.implode('<br>',$emsg_array1).'</div>';
				$_SESSION['emsg_array'] = array();
			}
			?>
			...
			<input type="text" name="user_email" id="user_email" class="input-text" value="<?php echo esc_attr(wp_unslash($user_email)); ?>" size="25" /></label>
			</p>
			
			<p class="form-row form-row-first">
			<label for="password">Password<br/>
			<input id="password" class="input-text" type="password" size="25" value="" name="password" /></label>
			</p>	
			<p class="form-row form-row-last">
			<label for="repeat_password">Repeat password<br/>
			<input id="repeat_password" class="input-text" type="password" size="25" value="" name="repeat_password" /></label>
			</p>
			<?php
			/**
			...
			<p class="cart" style="text-align:right;"><input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e('Register'); ?>" /></p>
			</form>
			<?php		
		}else{?>		
...
function aheadzen_register_site_process_indicator()
{
	...
	$shop_page_url = get_permalink( woocommerce_get_page_id( 'shop' ) ).'/?msg=theme-select';
	?>
	<div class="signup_selected_template">You have selected "<?php echo get_the_title($_SESSION['my_templateid']);?>" to create your site. Please <a href="<?php echo $shop_page_url;?>">select new template site</a> if you want to change.</div>
	<?php }?>
	<?php
}
...
function woocommerce_short_description_fun($description)
{
	...
	/*$link_url =  $signup_data['url'].'?my_template='.$mobile_app_theme;*/
	$link_url =  $signup_data['url'].'?my_templateid='.$pid;
	if($mobile_app_theme){
		$description .= '<div class="singup_continue_button"><a href="'.$link_url.'" class="btn btn-large  btn-acid-grey-4">'.__("Signup & Continue >>",'aheadzen').'</a></div>';
	}	
	return $description;
}
