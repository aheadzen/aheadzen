...
$str += '	<a href="" title="" class="style-picker-ico"> <img src="'+theme_folder_url+'images/style-picker/picker-icon.png" alt="" title="" /> </a>';
$str += '	<div id="dt-style-picker">';
$str += '   	<h2> Select Your Style </h2>';
$str += '       <h3> Choose your layout </h3>';
$str += '		<ul class="layout-picker">';
$str += '       	<li> <a id="fullwidth" href="" title="" class="selected"> <img src="'+theme_folder_url+'images/style-picker/fullwidth.jpg" alt="" title="" /> </a> </li>';
$str += '       	<li> <a id="boxed" href="" title=""> <img src="'+theme_folder_url+'images/style-picker/boxed.jpg" alt="" title="" /> </a> </li>';
$str += '		</ul>';
...
$str += '		</ul>';
$str += '			<div class="hr"> </div>';
$str += '	<center><button type="button" class="imgbutton_all editingbutton">Upload Images</button></center>';
//$str += '	<center><a href="'+widget_customization_url+'"><button type="button" class="editingbutton">Manage Widgets</button></a></center>';
$str += '	</div>';
...
jQuery(document).ready(function($){
	...
	if($.cookie("mytheme_layout") == "boxed"){
		...
		$img = 	theme_folder_url+"images/patterns/"+$i+".jpg";
	...
	var mytheme_skin = '';
	if($.cookie("mytheme_skin")!= null){
		mytheme_skin = $.cookie("mytheme_skin");		
	}
	if($.cookie("mytheme_layout") == "boxed"){
		mytheme_layout = $.cookie("mytheme_layout");		
	}
	if($.cookie("mytheme_pattern")!= null){
		mytheme_pattern = $.cookie("mytheme_pattern");		
	}
	
	jQuery.ajax({
	type : 'POST',
	url : site_admin_url,
	data : { action: 'setdisplayskin', skin: mytheme_skin, pattern: mytheme_pattern, layout: mytheme_layout }, // Add further params if you need to
	success : function(r) {
		//your ajax code here
		}
	});
	...
	$("ul.layout-picker a").click(function(e){
		...
		if( $.cookie("mytheme_layout") === "boxed") {
			...
			if( $.cookie("mytheme_pattern") == null ){
				...
				$("ul.pattern-picker a[id="+$.cookie("mytheme_pattern")+"]").addClass('selected');
				$img = 	theme_folder_url+"images/patterns/"+$.cookie("mytheme_pattern")+".jpg";
		...
		var mytheme_layout = $this.attr("id");
			jQuery.ajax({
				type : 'POST',
				url : site_admin_url,
				data : { action: 'setdisplayskin', layout: mytheme_layout }, // Add further params if you need to
				success : function(r) {
					//your ajax code here
	...
	$("ul.pattern-picker a").click(function(e){
		if($.cookie("mytheme_layout") == "boxed"){
			...
			$("ul.pattern-picker a").removeAttr("class");
			$this.addClass("selected");
			var mytheme_pattern = $this.attr("id");
			jQuery.ajax({
				type : 'POST',
				url : site_admin_url,
				data : { action: 'setdisplayskin', pattern: mytheme_pattern }, // Add further params if you need to
				success : function(r) {
					//your ajax code here
					//$.cookie("mytheme_pattern", $this.attr("id"), { path: '/' });
					$img = 	theme_folder_url+"images/patterns/"+$.cookie("mytheme_pattern")+".jpg";
	...
	$("ul.color-picker a").click(function(e){
		...
		$this.addClass("selected");
		var mytheme_skin = $this.attr("id");
		jQuery.ajax({
			type : 'POST',
			url : site_admin_url,
			data : { action: 'setdisplayskin', skin: mytheme_skin }, // Add further params if you need to
			success : function(r) {
				//your ajax code here
