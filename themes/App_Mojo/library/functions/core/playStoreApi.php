	...
	class PlayStoreApi{
		...
		function itemInfo($item_id){
			...
			phpQuery::newDocumentHTML($this_content[0]);
				...
				$cover_image =pq('.cover-container > img.cover-image ')->attr('src');;
				$banner_icon = pq('.doc-banner-icon > img')->attr('src');
				$ratings_context = explode(' ',pq('.header-star-badge > .stars-container > .tiny-star')->attr('title'));
				if(isset($ratings_context[1])){$ratings = $ratings_context[1];}else{$ratings = 'Not defined';}
				$app_title = pq('.info-container > .document-title > div')->html();
				$app_author = pq('a.document-subtitle.primary > span')->html();
				$app_category = pq('a.document-subtitle.category > span')->html();
				$author_store_url = $this->base_store_url.''.pq('a.document-subtitle.primary')->attr('href');
				$app_price = pq('.buy-button-price')->html();
				if($app_price == 'Install') { $app_price = 'Free'; }
				$html_app_description = pq('.show-more-content > .id-app-orig-desc')->html();
				$text_plain_app_description = strip_tags($html_app_description);
	
				foreach(pq('.screenshots .thumbnails > img') as $appshots){
					$app_screen_shots = pq($appshots)->attr('src');
					//$app_info['ScreenShots'][] = (object) array('screen_shot' => $app_screen_shots);
					$app_info['ScreenShots'][] = $app_screen_shots;
				}
				$app_last_updated = pq('.details-section-contents > .meta-info > .content[itemprop="datePublished"]"]')->html();
				//$app_last_updated = pq('time[itemprop="datePublished"]')->html();
				//$software_version = pq('dd[itemprop="softwareVersion"]')->html();
				$software_version = pq('.details-section-contents > .meta-info > .content[itemprop="softwareVersion"]')->html();
				$software_rating = pq('.details-section-contents > .meta-info > .content[itemprop="contentRating"]')->html();
				$os_required = 'Android '.pq('dt[itemprop="operatingSystems"] + dd')->html();
				//$file_size = pq('dd[itemprop="fileSize"]')->html();
				$file_size = pq('.details-section-contents > .meta-info > .content[itemprop="fileSize"]')->html();
				...
				$whats_new_html = pq('.whatsnew > .details-section-contents > .recent-change')->html();
				...
				$app_info['General'][] = (object) array(
						...
						'app_price' => $app_price,
						'html_app_description' => $html_app_description,
						...
						'app_permission_html' => $app_permission_html,
						'app_permission_text_plain' => $app_permission_text_plain,
						'whats_new_html' => $whats_new_html,
						'whats_new_text_plain' => $whats_new_text_plain,
